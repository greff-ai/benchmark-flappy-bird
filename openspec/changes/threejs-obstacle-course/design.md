...
- Expose only cloned observation snapshots for browser tests. Tests can use observations to time real Playwright key presses or touch taps, but cannot inject positions, set score, disable collisions, or call simulation commands. Adapt the existing PNG color/position checks to bird/pipes and retain resize/error assertions.
