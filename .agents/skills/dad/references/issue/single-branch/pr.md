...
4. Fetch origin. Require the container branch to be published and
...
6. Move the issue to active and append its working branch once to the tracker
   and local mirror. Commit scoped changes. Branch checkpoint complete.
...
7. Commit and push the child. A failed push leaves the commit intact and
   ...
   in ISSUE_DIR/pr.md, commit, and push that record before merging.
8. Move the issue to review; PR checkpoint complete. Follow
...
on that branch, push best-effort, and return to the container branch. Preserve
