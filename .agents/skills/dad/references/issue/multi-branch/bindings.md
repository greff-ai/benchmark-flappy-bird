...
1. Require clean tree on the sprint parent, not MAIN_BRANCH. Parse the row
   ...
   no parent (this is the only decomposition level) and its mirror on origin.
...
3. Move the issue to active; append the working branch once and commit/push
   its mirror. Do not create an OpenSpec change for the container itself.
...
7. On success return the shared child report on the sprint branch, with PR
   and ISSUE_DIR/tasks.md as the nested evidence. On halt commit only owned
   work on this issue branch, push best-effort, and return clean to the sprint.
