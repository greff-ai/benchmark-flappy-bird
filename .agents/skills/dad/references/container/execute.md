...
1. Require a clean tree on the container branch; fetch origin and reconcile
   ...
   prior failed push. Divergence halts CONFLICT. Validate ledger identity and
...
3. Walk each unchecked row once this run. Validate state and size; malformed
   ...
   and checkpoint, commit if changed, and continue.
4. View the issue with comments. A missing/unreadable issue halts this row.
   ...
   and record running. Commit/push the mirror and ledger before branching.
...
8. Commit/push changed ledger and owned bookkeeping files, skipping empty
   commits; then continue to the next independent row. Do not immediately
