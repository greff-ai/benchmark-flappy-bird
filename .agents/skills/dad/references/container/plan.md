...
3. Load the ledger, or create its header from the bindings. Validate identity
...
11. Append complete keyed rows to the ledger. Mirror the container issue,
    commit only changed owned paths, and push before execution. Skip empty
    commits. Report added/retained rows, links, and warnings. A planning
